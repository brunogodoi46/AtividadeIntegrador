...
$('#id-form-novo-pedido').on('submit', function (event) {
    ...
    // acria uma requisição assincrona
    $.ajax({
        type: 'POST',
        ...
        success: function (response) {
            ...
            response.return.forEach(element => {
                $("#list-pedidos").append(`<a href="#" class="list-group-item list-group-item-action" 
                data-toggle="list" value=${element.id}>Pedido ${element.id}</a> `);
            });
        ...
        // Quando o servidor n(ao consegue responder)
